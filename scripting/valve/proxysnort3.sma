...
new g_hud_sync, g_log_path[128], g_whitelist_path[128];

new g_names[MAX_HUD_ENTRIES][MAX_PLAYERS];
new g_risks[MAX_HUD_ENTRIES];
new g_info[MAX_HUD_ENTRIES][MAX_RESOURCE_PATH_LENGTH];
new g_entry_count = 0;
...
new g_original_name[MAX_PLAYERS + 1][MAX_PLAYERS];
...
public plugin_init()
{
    ...
    new config_dir[MAX_RESOURCE_PATH_LENGTH];
    get_configsdir(config_dir, charsmax(config_dir));
    formatex(g_log_path, charsmax(g_log_path), "%s/proxy_sniff.json", config_dir);
...
public client_connect(id)
{
    ...
    if (is_user_bot(id) || g_is_whitelisted[id]) return;

    new ip[MAX_PLAYERS];
    get_user_ip(id, ip, charsmax(ip), 1);
    if (!equal(ip, "127.0.0.1")) start_proxy_check(id, ip);
}
...
public fwd_ClientUserInfoChanged(id, infobuffer)
{
    ...
    new current_name[MAX_PLAYERS], target_name[MAX_PLAYERS];
...
{
    new error, token[MAX_RESOURCE_PATH_LENGTH], request[256], authid[MAX_PLAYERS], modname[MAX_PLAYERS];
    get_pcvar_string(g_cvar_token, token, charsmax(token));
    get_user_authid(id, authid, charsmax(authid));
    get_modname(modname, charsmax(modname));

    new socket = socket_open("proxycheck.io", 80, SOCKET_TCP, error);
    if (socket > 0)
    {
...
public parse_json_response(const data[], socket)
{
    ...
    if (ip_obj != Invalid_JSON)
    {
        ...
        {
            new country[MAX_PLAYERS], isp[MAX_RESOURCE_PATH_LENGTH], attack_reason[MAX_PLAYERS];
            ...
            {
                if (json_object_has_value(history, "Vulnerability Probing")) copy(attack_reason, charsmax(attack_reason), "Vulnerability Probing");
                else if (json_object_has_value(history, "Login Attempt")) copy(attack_reason, charsmax(attack_reason), "Brute Force");
                else if (json_object_has_value(history, "Comment Spam")) copy(attack_reason, charsmax(attack_reason), "Spam Bot");
                json_free(history);
            }
...
public execute_troll(id, const ip[], risk, const country[], const isp[], const reason[])
{
    ...
    {
        copy(g_names[i], 31, g_names[i-1]);
        copy(g_info[i], 63, g_info[i-1]);
        g_risks[i] = g_risks[i-1];
    }
    
    copy(g_names[0], 31, g_original_name[id]);
    formatex(g_info[0], 63, "%s (%s)", country, reason);
    g_risks[0] = risk;
    ...
    new troll_name[MAX_PLAYERS];
    get_troll_name(g_active_troll[id], troll_name, charsmax(troll_name));
    
    new Float:origin[3];
    pev(id, pev_origin, origin);
    
    message_begin(MSG_BROADCAST, SVC_SPAWNSTATICSOUND);
    ...
    new kick_data[MAX_RESOURCE_PATH_LENGTH]; copy(kick_data, charsmax(kick_data), reason);
...
{
    new JSON:root = file_exists(g_log_path) ? json_parse(g_log_path, true) : json_init_array();
    ...
    for (new i = 0; i < json_array_get_count(root); i++) {
        ...
        if (json_object_get_number(temp, "timestamp") < iCutoff) { json_array_remove(root, i--); }
        json_free(temp);
    }

    new JSON:entry = json_init_object();
    new name[MAX_PLAYERS], t[MAX_PLAYERS];
...
public cmd_show_proxies(id)
{
    ...
    if (g_entry_count == 0) return PLUGIN_HANDLED;
...
public bool:check_file_whitelist(id)
{
    ...
    new authid[MAX_PLAYERS], line[MAX_PLAYERS], file = fopen(g_whitelist_path, "rt");
    get_user_authid(id, authid, charsmax(authid));
    if (file) {
        while (!feof(file)) {
...
public cmd_whitelist_add(id, level, cid)
{
    ...
    new arg[MAX_PLAYERS]; read_argv(1, arg, charsmax(arg));
    new target = cmd_target(id, arg, 0);
    if (target) {
        new authid[MAX_PLAYERS], name[MAX_PLAYERS]; get_user_authid(target, authid, charsmax(authid));
