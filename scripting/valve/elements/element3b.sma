 ...
 *                                       
 *					DEBUGGING NOTES: needs thunderclap added as rain ambience on a timer and sometimes double clap etc. Wind...
 * CL_COMMANDS
 *
...
#define AUTHOR	"SPiNX" //inspired by TeddyDesTodes' Automatic weather
#define VERSION	".3b"
...
new g_skynames[][] = { "sunny","3dm_bikini","ava","sunset1","52h03", //SUNNY
...
//	g_cvar_appkey	= register_cvar("appkey","&APPID=04cfbf74c089b9d44e1a32ccb4e5cf83");
...
public showinfo(id) {
	...
	set_hudmessage (250, 120, 200, -1.0, 0.55, 1, 2.0, 3.0, 0.7, 0.8, -1); //pink bottom center first 3 are color next x and y next effect and time
...
//could be line(s) to fix South American issues
...
public read_web() {
	...
	if(!equal(buf,"")) {
		...
		if(containi(buf,"lat")>= 0){
			new out[32];
			copy(out,3,buf[strfind(buf,"lat")+29]); // some cities wont work
                        replace(out,3,"&","");
                        replace(out,3,"#","");
			#if defined DEBUG
			log_amx("Code: %s",out);
...
//	dusk = nvault_get(g_vault,"sunset");
//	dawn = nvault_get(g_vault,"sunrise");
	dusk = 21;
	dawn = 7;
	noon = 12;	
	get_time("%H",temp,2);
...
public precache_sky(const skyname[]) {
	...
	if(pres)
        ...
        //log_amx("sky is %s", skyname);

}
public daylight() {
	new sunset = 21 + 1; //g_sunset +1
	new sunrise = 6 - 1; //g_sunrise -1
	...
	if(now < noon){
		if(now < sunrise){
			...
			//log_amx("%d %d %d %d",lightspan,prenoon,lightspan/prenoon,now);
		}
	}
	if(now == noon){
	...
	if(now > noon){	
		if(now > sunset){
			...
			//light = get_cvar_num("lums")-(now)*(minlight-get_cvar_num("lums"))/(g_sunSet[0]-noon)))
	...
	//log_amx("%d %d %d",get_cvar_num("dark"),light,get_cvar_num("lums"));
	#endif
