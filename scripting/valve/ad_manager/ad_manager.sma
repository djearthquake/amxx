...
new const AUTHOR[] = "SPiNX|MaximusBrood"; //stability fix. Siezed/wedge server.
new g_iHeadcount;
...

{
    ...
    {
        emessage_begin(MSG_ONE_UNRELIABLE, gmsgSayText, {0,0,0}, params[2]);

        ewrite_byte(params[2]);

        ewrite_string(message);

        emessage_end();
    }
    //Display message to all //SPINX 2021
    else
    {
        if( iPlayers() > 0 )
        {
            emessage_begin(MSG_BROADCAST, gmsgSayText, {0,0,0}, 0);
...

{
        ...
        get_players(players,pNum,"ch") //h translates to jk_botti on op4
        g_iHeadcount = pNum;
