...
public del_hook(id)
{
    ...
    if (get_pcvar_num(pHead) > 2 && get_pcvar_num(pHead) <= 8) //tested works can detach hook from monsters 'unleashed'
    {
        // Remove players hook
        if (!canThrowHook[id])
            remove_hook(id)
    ...
    else if (get_pcvar_num(pHead) <=2 || get_pcvar_num(pHead) == 7)
...
public fwTouch(ptr, ptd)
{
    ...
    if (equali(szPtrClass, "Hook") || (get_pcvar_num(pPlayers) > 2 && equali(szPtrClass, "weapon_grapple")) )
    {
        ...
        if (pev_valid(ptd))
        {
            ...
            /*if(equali(szPtrClass, "Hook") || get_pcvar_num(pPlayers) > 2 && equali(szPtrClass, "weapon_grapple") && get_pcvar_num(pUseButtons < 0))
...
public throw_hook(id)
{
    ...
    switch(get_pcvar_num(pHead))
    {
        ...
        case  4: Hook[id] = create_entity("monster_snark")
        ...
        case  8: Hook[id] = create_entity("light")
        case  9: Hook[id] = create_entity("monster_tripmine")
