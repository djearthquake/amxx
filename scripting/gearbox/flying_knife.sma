...
#define CROW "fly_knife"

//precache
static blood_drop, blood_spray,trail;

static m_pPlayer,m_flNextSecondaryAttack;
static knife_render,damage_knife,knife_speed,knife_trail,knife_lifetime;
...
public plugin_init() {
    ...
    RegisterHam(Ham_Weapon_SecondaryAttack,"weapon_knife","fw_CrowbarSecondaryAttack")
    RegisterHam(Ham_Item_AddToPlayer,"weapon_knife","fw_CrowbarItemAdd")
    RegisterHam(Ham_Item_AddDuplicate,"weapon_knife","fw_CrowbarItemAdd")
    ...
    knife_lifetime = register_cvar("fly_knife_time","15.0")
    knife_speed = register_cvar("fly_knife_speed","1300")
    knife_render = register_cvar("fly_knife_render","1")
    knife_trail = register_cvar("fly_knife_trail","1")
    damage_knife = register_cvar("fly_knife_damage","240.0")
...
public FlyCrowbar_Touch(toucher,touched){
    ...
    if(!is_user_connected(touched)){
        ...
        ExecuteHamB(Ham_TakeDamage,touched,toucher,pev(toucher,pev_owner),get_pcvar_float(damage_knife),DMG_CLUB)
    ...
    new crow = engfunc(EngFunc_CreateNamedEntity,engfunc(EngFunc_AllocString,"weapon_knife"))
...
public FlyCrowbar_Spawn(id){
    new knife = engfunc(EngFunc_CreateNamedEntity,engfunc(EngFunc_AllocString,"info_target"))

    if(!pev_valid(knife))
        return 0

    set_pev(knife,pev_classname,CROW)
    engfunc(EngFunc_SetModel,knife,"models/w_knife.mdl")
    engfunc(EngFunc_SetSize,knife,Float:{-4.0, -4.0, -4.0} , Float:{4.0, 4.0, 4.0})

    new Float:vec[3]
    get_projective_pos(id,vec)
    engfunc(EngFunc_SetOrigin,knife,vec)
    ...
    set_pev(knife,pev_owner,id)
    set_pev(knife,pev_angles,vec)

    velocity_by_aim(id,get_pcvar_num(knife_speed)+get_speed(id),vec)
    set_pev(knife,pev_velocity,vec)

    set_pev(knife,pev_nextthink,get_gametime()+0.1)

    DispatchSpawn(knife)

    if(get_pcvar_num(knife_trail)){
    ...
    set_pev(knife,pev_movetype,MOVETYPE_TOSS)
    set_pev(knife,pev_solid,SOLID_BBOX)

    emit_sound(id,CHAN_WEAPON,"weapons/cbar_miss1.wav",0.9,ATTN_NORM,0,PITCH_NORM)
    set_task(0.1,"FlyCrowbar_Whizz",knife)
...
public FlyCrowbar_Whizz(knife){
    if(pev_valid(knife)){
        emit_sound(knife,CHAN_WEAPON,"weapons/cbar_miss1.wav",0.9,ATTN_NORM,0,PITCH_NORM)

        set_task(0.2,"FlyCrowbar_Whizz",knife)
    }
}
