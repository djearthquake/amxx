...
#metamod
LOC="+localinfo mm_gamedll dlls/$SERVICE"
...
else
{
    ...
    nohup screen -A -m -d -S $MOD ./hlds_run +ip "${SERVER_IP}" $BOOST -game $MOD $MAP $EDI $ZONE $META $LOC $NET $PORT $CFG +maxplayers $MAX_PLAYERS +mapchangecfgfile server.cfg +hostname $NAME$ID -heapsize 1024000 -update > /dev/null &
