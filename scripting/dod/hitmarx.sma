...
new g_cvar_iHitSym;
new pRainbow,pSpinners,pMarkerRed,pMarkerGRN,pMarkerBLU,pHUDx,pHUDy,pLinger;
...
public newlog(id){
	...
	if(is_user_admin(id)){
	client_print(id, print_chat, "amx_rcon hitmarx 1-8");}
	@demo(id);
}

@PostTakeDamage(iVictim, iInflictor, iAttacker, Float:iDamage, iDamagebits)
{
	if (get_pcvar_num(g_cvar_iHitSym) >= 0 && !is_user_bot(iAttacker))
	if (is_user_connected(iAttacker) && get_pcvar_num(g_cvar_iHitSym) >= 1)
	{
	if (get_pcvar_num(g_cvar_iHitSym) == 1 && is_user_connected(iAttacker) && CSW_ALL_SNIPERRIFLES & (1 << get_user_weapon(iAttacker)) || //cstrike only works so far tested
		 (get_pcvar_num(g_cvar_iHitSym) >= 2 && ( cstrike_running() || (is_running("dod") == 1) ) && (get_user_team(iVictim) != get_user_team(iAttacker))  || !cstrike_running()) ) //makes gearbox mod work if !cstrike
{
	new iRed, iGRN, iBLU;
